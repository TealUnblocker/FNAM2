...
class EnemyAI {
    ...
    start() {
        ...
        if (this.currentTrumpConfig && this.trump.aiLevel > 0) {
            console.log(`Night ${this.game.state.currentNight} - Mikey AI Config:`, this.currentTrumpConfig);
            console.log(`Mikey will spawn in ${this.currentTrumpConfig.spawnDelay / 1000} seconds...`);
    ...
    loadAIConfig() {
        ...
        if (this.game.state.customNight && night === 7) {
            ...
            console.log(`- Mikey: Level ${this.trump.aiLevel || 0}`);
        ...
        if (this.currentTrumpConfig) {
            console.log(`- Mikey: Level ${this.trump.aiLevel}, Interval ${this.trump.movementInterval}ms`);
        } else {
            console.log(`- Mikey: Not active this night`);
        }
    }
    ...
    spawnTrump() {
        ...
        console.log('Mikey has spawned at cam10!');
    ...
    moveTrumpToNextLocation() {
        ...
        if (this.game.state.currentNight === 5 && this.game.state.currentTime >= 4) {
            ...
            if (!this.trump.night5AggressiveMode) {
                console.log('⚡ Night 5: 4AM reached! Mikey is now in aggressive mode (faster + more crawling)');
        ...
        if (this.trump.isCrawling) {
            console.log('Mikey is crawling, cannot move');
            return;
        }
        ...
        if (currentLoc === 'cam1') {
            ...
            if (shouldCrawl) {
                console.log(`Mikey starting to crawl from ${currentLoc} to office (${config.ventCrawling.cam1Probability * 100}% chance)`);
        ...
        if (currentLoc === 'cam2') {
            ...
            if (shouldCrawl) {
                console.log(`Mikey decided to crawl from ${currentLoc} to office (${config.ventCrawling.cam2Probability * 100}% chance)`);
                this.startTrumpCrawling(currentLoc);
                return;
            } else {
                console.log(`Mikey decided to continue moving from ${currentLoc} (${(1 - config.ventCrawling.cam2Probability) * 100}% chance)`);
                // 继续执行正常移动逻辑
            }
        }
        ...
        if (forwardLocations.length === 0 && lateralLocations.length === 0 && backwardLocations.length === 0) {
            console.log(`Mikey has no valid path from ${currentLoc}`);
            return;
        }
        ...
        // 如果总概率为0，不移动
        if (totalProb === 0) {
            console.log(`Mikey movement probability is 0`);
            return;
        }
        ...
        console.log(`Mikey moved [${movementType}]: ${currentLoc} (depth ${currentDepth}) -> ${nextLocation} (depth ${this.trumpLocationDepth[nextLocation]})`);
    ...
    startTrumpCrawling(fromLocation) {
        ...
        if (this.game.state.ventsClosed) {
            console.log('Mikey tried to crawl but vents are already closed! Silent retreat.');
            ...
            console.log(`Mikey silently retreats to ${retreatLocation} (depth 3)`);
        ...
        console.log(`Mikey is crawling from ${fromLocation}...`);
        ...
        this.trump.crawlingTimer = setTimeout(() => {
            console.log('Mikey reached the office!');
    ...
    stopTrumpCrawling() {
        ...
        console.log('Mikey crawling blocked by closed vents!');
        ...
        console.log(`Mikey will retreat to ${retreatLocation} (depth 3)`);
    ...
    attractToSound(soundLocation) {
        ...
        if (this.trump.hasSpawned && !this.trump.isCrawling && adjacentToTrump && adjacentToTrump.includes(soundLocation)) {
            ...
            console.log(`Mikey attracted by sound: ${trumpCurrentLoc} (depth ${currentDepth}) -> ${soundLocation} (depth ${soundDepth})`);
            ...
            console.log(`Sound at ${soundLocation} is not adjacent to Mikey at ${trumpCurrentLoc}`);
        }
